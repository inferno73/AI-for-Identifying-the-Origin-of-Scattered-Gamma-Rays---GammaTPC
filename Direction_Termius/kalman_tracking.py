...
class TrackKalmanFilter:
    ...
    def fit(self, points):
        ...
        # Use more points for robustness (e.g. 50 or all if short)
        n_fit = min(len(smoothed), 50)
        fit_segment = np.array(smoothed[:n_fit])
        
        if len(fit_segment) < 2: return np.array([0., 0., 1.]), smoothed
        
        # Least squares line fit
        mean = np.mean(fit_segment, axis=0)
        uu, dd, vv = np.linalg.svd(fit_segment - mean)
        direction = vv[0] # Principal component
        
        # DEBUG SVD check
        if dd[0] < 1e-3:
             # Degenerate? Fallback to global vector of the segment
             vec = fit_segment[-1] - fit_segment[0]
        ...
        global_vec = fit_segment[-1] - fit_segment[0]
