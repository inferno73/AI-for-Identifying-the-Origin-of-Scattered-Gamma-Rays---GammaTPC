...
def get_predicted_origin(points, features, anchor_point=None):
    ...
    Includes centering logic (Center [32,32,32]) for 64-voxel model.
    """
    if MODEL is None: return np.zeros(3)
    
    # Calculate Centroid / Reference
    if anchor_point is not None:
         center_ref = anchor_point
    else:
         center_ref = np.mean(points, axis=0) # (3,)
    
    # Center Points to range [-32, 32]
    centered_points = points - center_ref
    
    # Add Offset to map to [0, 64] box
    ...
    # Output is local offset from center
    p_org_local = pred_org.cpu().numpy()[0]
    
    # Reconstruct Global: CenterRef + Prediction
...
def sort_points_nn(coords, origin):
    ...
    # Compute distance from origin to all points
    dists_origin = np.linalg.norm(coords - origin, axis=1)
    
    # Identify candidates (Top 10 closest to origin)
    k_candidates = min(10, len(coords))
    candidate_indices = np.argsort(dists_origin)[:k_candidates]
    
    # Select best start: Furthest from track centroid (Extremity Heuristic)
...
if __name__ == "__main__":
    ...
    DEFAULT_OUT = "/sdf/home/b/bahrudin/gammaTPC/workfolder-zerinaa-new/AI_playground/0.direction/results_FULL"
