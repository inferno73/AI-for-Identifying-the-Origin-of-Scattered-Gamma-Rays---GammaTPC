...
def plot_track_3d_plotly(result, save_dir, prefix):
    ...
    # Plot Traces
    # 1. Track Points
    # Dynamic Marker Size based on Charge
    # Feats: 0=Charge(Log), 1=dQ, 2=Lin, 3=Dens
    # If using scale, marker size ref is pixels.
    # Log charge usually 3..8.
    marker_sizes = charges * 1.5 
    marker_sizes = np.clip(marker_sizes, 2, 10) # Min 2px, Max 10px blobs
...
def evaluate(args):
    ...
    try:
        ...
        if len(dataset) > 0:
            sample_feats = dataset[0][1] # (N, C)
            print(f"DEBUG: Dataset Feature Shape: {sample_feats.shape}")
            if sample_feats.shape[1] != 4:
                print(f"CRITICAL ERROR: Dataset has {sample_feats.shape[1]} features, but Model expects 4.")
                print("You are likely using an OLD dataset. Please regenerate with make_dataset_kf.py")
                sys.exit(1)
    ...
    if args.model_path and os.path.exists(args.model_path):
        ...
        if os.path.exists(model_path_cwd):
            ...
            print(f"CRITICAL ERROR: Model not found at any location.")
            print(f"Checked: {model_path_cwd}, {model_path_best}, and args.model_path")
            sys.exit(1)
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            for b in range(bs):
                ...
                results.append({
                    ...
                    'lbl_dir': lbl_dir[b].cpu().numpy(), # Critical for KF recalcn
                ...
                if len(core_points) > 5:
                    try:
                        ...
                        print(f"KF FAILED for Track {i}: {e}")
                        # Provide traceback
                        import traceback
                        traceback.print_exc()
