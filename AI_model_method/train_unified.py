...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            # Total Loss (Weight recon higher to force structure learning)
            loss = loss_org + loss_dir + 5.0 * loss_recon
            ...
            pbar.set_postfix({'loss': loss.item(), 'org': loss_org.item(), 'dir': loss_dir.item(), 'rec': loss_recon.item()})
       
        # Checkpointing
        epoch_avg_loss = total_loss / len(dataloader)
        epoch_avg_loss = total_loss / len(dataloader)
        epoch_avg_loss = total_loss / len(dataloader)
        print(f"Epoch {epoch+1} Loss: {epoch_avg_loss:.4f} (Org: {loss_org.item():.4f}, Dir: {loss_dir.item():.4f}, Rec: {loss_recon.item():.4f})")
