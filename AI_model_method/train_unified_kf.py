...
# Mock spconv for CPU/Local debugging if unavailable
try:
    ...
    MOCK_MODE = True
    print(">> Spconv not found. Using Dense Fallback.")

    # Minimal Mock classes to mimic spconv API
    ...
    class MockLayer(nn.Module):
        ...
        def forward(self, input_tensor):
            # Dense fallback logic for debugging
            ...
            # Gather back to sparse tensor
            b_dim, c_dim, x_dim, y_dim, z_dim = out_dense.shape
            
            indices = torch.nonzero(torch.sum(torch.abs(out_dense), dim=1) > -1)
    ...
    class SparseInverseConv3d(MockLayer): 
        def __init__(self, in_channels, out_channels, kernel_size, indice_key=None, bias=True):
            super().__init__(in_channels, out_channels, kernel_size, stride=1, padding=0, bias=bias, indice_key=indice_key) 
            if isinstance(kernel_size, int): kernel_size = (kernel_size,)*3
...
class GammaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Quantize Coords: Add offset to make indices positive [0, spatial_size]
        ...
        # 0: Charge (Log1p), 1: dQ (ArcSinh), 2: Linearity (0-1), 3: Density (Log1p)
